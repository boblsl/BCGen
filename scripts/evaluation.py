...
def nltk_bleu_old(hypotheses, references, output_path):
    ...
    for hyp, ref in zip(hypotheses, references):
        ...
        if len(hyp) >= 4:
            try:
                ...
                # output_file.write('ROUGE:.4f%' % rouge_score + '\n')
                #
    ...
    avg_bleu4 = total_bleu4 / count
    print('avg_bleu4: %.4f' % avg_bleu4)
    output_file.write('avg_score: %.4f' % avg_score + '\n')
    output_file.write('avg_bleu1: %.4f' % avg_bleu1 + '\n')
    output_file.write('avg_bleu2: %.4f' % avg_bleu2 + '\n')
    output_file.write('avg_bleu3: %.4f' % avg_bleu3 + '\n')
    output_file.write('avg_bleu4: %.4f' % avg_bleu4 + '\n')
...
def evaluate(reference, predictions, output_path):
    ...
    return nltk_bleu_old(hypotheses, references, output_path)

if __name__ == '__main__':
    # evaluate(sys.argv[1], sys.argv[1])

    evaluate('../output/refs_onlycfg.txt', '../output/hyps_onlycfg.txt', '../output/result_onlycfg.txt')